...
traffic_df = traffic_df.withColumn("total_rev", round("total_rev",1))
display(traffic_df)
...
# TODO
top_traffic_df = (traffic_df.orderBy(col("total_rev").desc())
).limit(3)
...
final_df = (
    top_traffic_df.withColumn("avg_rev", col("avg_rev")*100)
            .withColumn("total_rev", col("total_rev")*100)
            .withColumn("avg_rev",col("avg_rev").cast(LongType()))
            .withColumn("total_rev",col("total_rev").cast(LongType()))
...
# TODO
chain_df = (
  (df.groupBy("traffic_source").agg(sum("revenue").alias("total_rev"), avg("revenue").alias("avg_rev"))
    ).withColumn("total_rev", round("total_rev",1)).orderBy(col("total_rev").desc()).limit(3)
).withColumn("total_rev",round("total_rev",2)).withColumn("avg_rev",round("avg_rev",2))
